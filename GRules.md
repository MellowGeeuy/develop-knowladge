...
- บท TypeScript: บรรทัดที่ `tsc` ฟ้องเขียน `// ❌ ข้อความ` โดยคัดลอกข้อความจาก tsc ตรงตัว บรรทัดอื่นในบล็อกต้องไม่มี error
  · type ที่เดาได้เขียน `// ชนิด: T` ให้ตรงกับที่เอาเมาส์ชี้แล้วเห็น · ถ้าต้องคลี่ union ให้อ่านง่ายใช้ `// เท่ากับ ...`
- ทุกบล็อกโค้ดต้องคัดลอกไปรันได้เอง (มี type และค่าที่ใช้ครบในบล็อก) ตัวอย่างหลายไฟล์ใช้ `file=` ให้เห็นว่าเป็นไฟล์ไหนในโปรเจกต์
